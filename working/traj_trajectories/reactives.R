...
temporaImport <- reactive({
  ...
  colData(scEx_log)[,tCluster] = factor(colData(scEx_log)[,tCluster])
  # SingleCellExperiment returns a DataFrame, but Tempora wants a data.frame
  setMethod("getMD","SingleCellExperiment",
            function(x) SingleCellExperiment::colData(x) %>% as.data.frame())
...
temporaTrajectory <- reactive({
  ...
  # temporaObj = to
  temporaObj = tryCatch ({
    BuildTrajectory(object = temporaObj, 
                    n_pcs = n_pcs, 
